...
class SchedulerService:
    def __init__(self):
        ...
        scheduler.add_job(
            self.scheduler_notification, "cron", hour="0", minute="0", day="*"
        )
        scheduler.start()
        ...
        async with async_session() as self.session:
            ...
            for result in results:
                ...
                company = await self.session.execute(
                    select(Company).filter(Company.id == quiz.company_id)
                )
                ...
                if user_time > quiz_time:
                    notification = Notification(
                        user_id=user_id,
                        message=f"You took the quiz {quiz.name} late in {company_name} company",
                        time=datetime.utcnow(),
                    )
                    result = await self.session.execute(
                        select(Notification).filter(
                            Notification.user_id == user_id,
                            Notification.message == notification.message, # noqa
                        )
                    )
