...
from app.schemas.schemas import (SignUpRequestModel, 
...
async def update(user_up: UserPersonalEdit, session: AsyncSession=Depends(get_db_session), current_user: AuthResponse=Depends(get_current_user)):
    return await UserService(session).update_user(user_id=current_user.get('detail').id, user_up=user_up)

@router.delete("/delete", response_model=UserDelatedResponse)
async def remove(session: AsyncSession=Depends(get_db_session), current_user: AuthResponse=Depends(get_current_user)):
