...
async def invite_user(company_id: int, user_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await CompanyActions(session).invite(company_id=company_id, user_id=user_id, current_user=current_user.get('detail').id)

@router.post("/invite/reject/{company_id}/{user_id}")
async def reject_invite(company_id: int, user_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await CompanyActions(session).reject_invite(company_id=company_id, user_id=user_id, current_user=current_user.get('detail').id)

@router.post("/accept/invite/{company_id}")
async def accept_invite(company_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await UserActions(session).accept_invite(company_id=company_id, user_id=current_user.get('detail').id)

@router.post("/decline/invite/{company_id}")
async def decline_invite(company_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await UserActions(session).decline_invite(company_id=company_id, user_id=current_user.get('detail').id)

@router.post("/join/company/{company_id}")
async def join_company(company_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await UserActions(session).request_to_company(company_id=company_id, user_id=current_user.get('detail').id)

@router.post("/join/cancel/{company_id}")
async def cancel_join(company_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await UserActions(session).cancel_request(company_id=company_id, user_id=current_user.get('detail').id)

@router.post("/join/accept/{company_id}/{user_id}")
async def accept_join(company_id: int, user_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await CompanyActions(session).accept_request(company_id=company_id, user_id=user_id, current_user=current_user.get('detail').id)

@router.post("/join/decline/{company_id}/{user_id}")
async def decline_join(company_id: int, user_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await CompanyActions(session).decline_request(company_id=company_id, user_id=user_id, current_user=current_user.get('detail').id)

@router.post("/kick/{company_id}/{user_id}")
async def kick_user(company_id: int, user_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await CompanyActions(session).kick(company_id=company_id, user_id=user_id, current_user=current_user.get('detail').id)

@router.post("/leave/{company_id}")
async def leave_company(company_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await UserActions(session).leave_company(company_id=company_id, user_id=current_user.get('detail').id)

@router.get("/get/invites")
async def get_invites(session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await UserActions(session).invites(user_id=current_user.get('detail').id)

@router.get("/get/requests")
async def get_requests(session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await UserActions(session).requests(user_id=current_user.get('detail').id)

@router.get("/get/members/{company_id}")
async def get_members(company_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await CompanyActions(session).employees(company_id=company_id, current_user=current_user.get('detail').id)

@router.get("/get/company/requests/{company_id}")
async def get_company_requests(company_id: int, session: AsyncSession = Depends(get_db_session), current_user: dict = Depends(get_current_user)):
    return await CompanyActions(session).requests(company_id=company_id, current_user=current_user.get('detail').id)
