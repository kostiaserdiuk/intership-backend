...
async def create_quiz(
    ...
    return await QuizService(session).create_quiz(
        quiz=quiz, company_id=company_id, current_user=current_user.get("detail").id
    )
...
async def update_quiz(
    ...
    return await QuizService(session).update_quiz(
        quiz=quiz, quiz_name=quiz_name, current_user=current_user.get("detail").id
    )
...
async def delete_quiz(
    ...
    return await QuizService(session).delete_quiz(
        quiz_name=quiz_name, current_user=current_user.get("detail").id
    )
...
async def quiz_passage(
    ...
    return await QuizPassage(session).quiz_passing(
        company_id=company_id,
        quiz_name=quiz_name,
        current_user=current_user.get("detail").id,
        quiz_passing=quiz_passing,
    )
...
async def get_statistics(
    ...
    return await QuizService(session).get_statistic_by_company(
        company_id=company_id, current_user=current_user.get("detail").id
    )
...
async def get_general_statistics(
    ...
    return await QuizService(session).get_general_statistic(
        current_user=current_user.get("detail").id
    )
...
async def export_user_results(
    ...
    if export_model == ExportModel.json or export_model == ExportModel.csv:
        return await QuizService(session).user_export_results(
            current_user=current_user.get("detail").id, export_model=export_model
        )
...
async def export_company_results(
    ...
    if export_model == ExportModel.json or export_model == ExportModel.csv:
        return await QuizService(session).admin_export_results(
            company_id=company_id,
            current_user=current_user.get("detail").id,
