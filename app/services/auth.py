...
async def get_current_user(
    session: AsyncSession = Depends(get_db_session), token: str = Depends(oauth_schema)
) -> dict:
    try:
        payload = jwt.decode(token.credentials, key=SECRET_KEY, algorithms=["HS256"])
        result = await session.execute(
            select(UserModel).filter(UserModel.username == payload.get("username"))
        )
        user = result.scalars().first()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        ...
        result = await session.execute(
            select(UserModel).filter(UserModel.email == payload.get("user-email"))
        )
        user = result.scalars().first()
        if not user:
            new_user = UserModel(
                username=None, email=payload.get("user-email"), password=None
            )
