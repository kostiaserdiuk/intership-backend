...
class NotificationService:
    ...
    async def quiz_notification(self, quiz_name: str, company_id: int):
        ...
        for employee in employees:
            new_notification = Notification(user_id=employee.user_id, message=f'You have new quiz {quiz_name} in {company_name} company, please pass it', time=now)
    ...
    async def check_notifications(self, user_id: int):
        result = await self.session.execute(select(Notification).filter(Notification.user_id == user_id, Notification.is_read == False))
        ...
        notifications_response = [NotificationShema(id=notification.id, message=notification.message) for notification in notifications]
        return {'notifications': notifications_response}
    
    async def read_notification(self, notification_id: int, user_id: int):
        notification = await self.session.execute(select(Notification).filter(Notification.id == notification_id, Notification.user_id == user_id))
