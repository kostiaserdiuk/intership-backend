...
class CompanyActions:
    ...
    async def invite(self, company_id: int, user_id: int, current_user: int):
        ...
        if company.owner_id != current_user:
            raise HTTPException(
                status_code=403, detail="You are not the owner of this company"
            )
        ...
        try:
            result = await self.session.execute(
                select(CompanyAction).filter(
                    CompanyAction.company_id == company_id,
                    CompanyAction.invite_user == user_id,
                )
            )
    ...
    async def accept_request(self, company_id: int, user_id: int, current_user: int):
        result = await self.session.execute(
            select(UserAction).filter(
                UserAction.join_request == company_id, UserAction.user_id == user_id
            )
        )
        ...
        try:
            result = await self.session.execute(
                select(Employees).filter(
                    Employees.company_id == company_id, Employees.user_id == user_id
                )
            )
    ...
    async def invited(self, company_id: int, current_user: int):
        ...
        result = await self.session.execute(
            select(CompanyAction).filter(CompanyAction.company_id == company_id)
        )
    ...
    async def requests(self, company_id: int, current_user: int):
        ...
        result = await self.session.execute(
            select(UserAction).filter(UserAction.join_request == company_id)
        )
    ...
    async def get_admins(self, company_id: int, current_user: int):
        ...
        result = await self.session.execute(
            select(Employees).filter(
                Employees.company_id == company_id, Employees.is_admin == True
            )
        )
...
class UserActions:
    ...
    async def invites(self, user_id: int):
        result = await self.session.execute(
            select(CompanyAction).filter(CompanyAction.invite_user == user_id)
        )
        invites = result.scalars().all()
        if not invites:
    ...
    async def requests(self, user_id: int):
        result = await self.session.execute(
            select(UserAction).filter(UserAction.user_id == user_id)
        )
        requests = result.scalars().all()
        if not requests:
