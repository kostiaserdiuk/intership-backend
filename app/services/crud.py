...
class CompanyService:
    ...
    async def create_company(
        self, company: CompanyCreateRequestModel, owner_id: int
    ) -> dict:
        try:
            new_company = CompanyModel(
                name=company.name, description=company.description, owner_id=owner_id
            )
    ...
    async def update_company(
        self, company_name: str, company_up: CompanyCreateRequestModel, user_id: int
    ) -> dict:
        result = await self.session.execute(
            select(CompanyModel).filter(CompanyModel.name == company_name)
        )
