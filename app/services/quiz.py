...
class QuizService:
    ...
    async def create_quiz(self, quiz: Quiz, company_id: int, current_user: int):
        ...
        result = await self.session.execute(
            select(Employees).filter(
                Employees.company_id == company_id, Employees.user_id == current_user
            )
        )
        user = result.scalars().first()
        if not user and company.owner_id != current_user:
            raise HTTPException(
                status_code=404, detail="You are not an employee of this company"
            )
        if company.owner_id != current_user and user.is_admin == False:
            raise HTTPException(
                status_code=403, detail="You are not allowed to create quizzes"
            )
        try:
            ...
            new_quiz = QuizModel(
                ...
                time_to_pass=datetime.strptime(
                    quiz.time_to_pass, "%Y-%m-%d %H:%M:%S.%f"
                ),
            )
            ...
            await notification.quiz_notification(
                quiz_name=quiz.name, company_id=company_id
            )
    ...
    async def update_quiz(self, quiz: Quiz, quiz_name: str, current_user: int):
        result = await self.session.execute(
            select(QuizModel).filter(QuizModel.name == quiz_name)
        )
        ...
        try:
            ...
            quiz_model.time_to_pass = datetime.strptime(
                quiz.time_to_pass, "%Y-%m-%d %H:%M:%S.%f"
            )
            json_questions = jsonable_encoder(quiz.questions)
            for question in json_questions:
    ...
    async def get_quizzes(self, company_id: int, current_user: int):
        ...
        result = await self.session.execute(
            select(QuizModel).filter(QuizModel.company_id == company_id)
        )
        ...
        quizzes = [
            Quiz(
                ...
                time_to_pass=datetime.strftime(
                    quiz.time_to_pass, "%Y-%m-%d %H:%M:%S.%f"
                ),
            )
            for quiz in quizzes
    ...
    async def get_statistic_by_company(self, current_user: int, company_id: int):
        result = await self.session.execute(
            select(Rating).filter(
                Rating.user_id == current_user, Rating.company_id == company_id
            )
        )
        ...
        if not ratings:
            raise HTTPException(
                status_code=404,
                detail="You have not passed any quizzes in this company",
            )
    ...
    async def get_general_statistic(self, current_user: int):
        result = await self.session.execute(
            select(Rating).filter(Rating.user_id == current_user)
        )
        ratings = result.scalars().all()
        if not ratings:
            raise HTTPException(
                status_code=404, detail="You have not passed any quizzes"
            )
    ...
    async def user_export_results(self, current_user: int, export_model: str):
        ...
        user_data = [
            {key: eval(await r.get(key))}
            for key in await r.keys()
            if key.split(":")[0] == str(current_user)
        ]
        data_export = []
        for data in user_data:
            ...
            result = await self.session.execute(
                select(QuizModel).filter(QuizModel.id == int(quiz_id))
            )
        ...
        if export_model == "json":
            return (
                {"detail": "Your answers in the last 48 hours", "data": data_export}
                if len(data_export) > 0
                else {"detail": "You have no answers in the last 48 hours"}
            )
    ...
    async def admin_export_results(
        self, current_user: int, company_id: int, export_model: str
    ):
        result = await self.session.execute(
            select(Company).filter(Company.id == company_id)
        )
        ...
        if company.owner_id != current_user and user.is_admin == False:
            raise HTTPException(
                status_code=403, detail="You are not allowed to export results quizzes"
            )
        users_data = [
            {key: eval(await r.get(key))}
            for key in await r.keys()
            if key.split(":")[1] == str(company_id)
        ]
        data_export = []
        ...
        if export_model == "json":
            return (
                {"detail": "Users answers in the last 48 hours", "data": data_export}
                if len(data_export) > 0
                else {"detail": "Users have no answers in the last 48 hours"}
            )
        elif export_model == "csv":
            df = pd.DataFrame(data=data_export)
            stream = io.StringIO()
...
class QuizPassage:
    ...
    async def quiz_passing(
        self,
        quiz_passing: QuizPassing,
        quiz_name: str,
        company_id: int,
        current_user: int,
    ):
        ...
        result = await self.session.execute(
            select(QuizModel).filter(
                QuizModel.name == quiz_name, QuizModel.company_id == company_id
            )
        )
        quiz_model = result.scalars().first()
        if not quiz_model:
        ...
        if result_model:
            raise HTTPException(
                status_code=409, detail="You have already passed this quiz"
            )
        now = datetime.utcnow()
        result_model = Result(
            ...
            company_id=company_id,
            correct_answered=points,
            count_questions=len(correct_answers),
            time=now,
        )
        ...
        for user_answer, correct_answer, question in zip(
            user_answers, correct_answers, questions
        ):
            await r.setex(
                name=f"{current_user}:{company_id}:{quiz_model.id}:{question.get('question')}:{question.get('uuid')}",
                value=repr((user_answer, user_answer == correct_answer)),
                time=48 * 3600,
            )

        rating = await self.session.execute(
            select(Rating).filter(
                Rating.user_id == current_user, Rating.company_id == company_id
            )
        )
        ...
        if not rating_model:
            ...
            rating_model.scores = float(
                f"{((rating_model.correct_answered + points)/(rating_model.total_questions + len(correct_answers))):.2f}"
            )
