...
from app.schemas.schemas import (
    ...
    PassedQuiz,
    ScoreCompany,
    LastEmployeesPassage,
)
from app.schemas.schemas import (
    ScoresListResponse,
    RatingList,
    ScoresCompanyListResponse,
)
...
class AnaliticService:
    ...
    async def get_personal_rating(self, user_id: int):
        result = await self.session.execute(
            select(Rating).filter(Rating.user_id == user_id)
        )
        rating = result.scalars().all()
        if not rating:
            raise HTTPException(
                status_code=200, detail="You have not passed any quizzes yet"
            )
        user_rating = []
        for rate in rating:
            ...
            user_rating.append(
                RatingShema(
                    ...
                    avg_scores=rate.scores,
                    correct_answered=rate.correct_answered,
                    total_answered=rate.total_questions,
                )
            )
    ...
    async def get_general_rating(self):
        ...
        if not rating:
            raise HTTPException(
                status_code=200, detail="No one has taken any quizzes yet"
            )
        general_rating = []
        for rate in rating:
            result = await self.session.execute(
                select(User).filter(User.id == rate.user_id)
            )
            user_email = result.scalars().first().email
            general_rating.append(
                RatingShema(
                    user_email=user_email,
                    company_id=rate.company_id,
                    avg_scores=rate.scores,
    ...
    async def get_avg_scores_all_quizzes(self, user_id: int):
        result = await self.session.execute(
            select(Result).filter(Result.user_id == user_id)
        )
        ...
        for result in results:
            quiz = await self.session.execute(
                select(Quiz).filter(Quiz.id == result.quiz_id)
            )
    ...
    async def get_passed_quizzes(self, user_id: int):
        ...
        for result in results:
            ...
            quiz_name = quiz.scalars().first().name
            passed_quizzes.append(
                PassedQuiz(quiz_name=quiz_name, time=str(result.time))
            )
    ...
    async def get_scores_by_company(self, company_id: int, current_user: int):
        ...
        if company.owner_id != current_user and user.is_admin == False:
            raise HTTPException(
                status_code=403, detail="You are not admin or owner of this company"
            )
        result = await self.session.execute(
            select(Result).filter(Result.company_id == company_id)
        )
        results = result.scalars().all()
        if not results:
            raise HTTPException(
        ...
        for rate in results:
            quiz = await self.session.execute(
                select(Quiz).filter(Quiz.id == rate.quiz_id)
            )
            quiz_name = quiz.scalars().first().name
    ...
    async def get_user_scores_by_company(
        self, company_id: int, current_user: int, user_id: int
    ):
        result = await self.session.execute(
            select(Company).filter(Company.id == company_id)
        )
        ...
        result = await self.session.execute(
            select(Result).filter(
                Result.company_id == company_id, Result.user_id == user_id
            )
        )
        results = result.scalars().all()
        if not results:
            raise HTTPException(
                status_code=200, detail="User has taken any quizzes yet"
            )
        ...
        for rate in results:
            ...
            max_scores = rate.count_questions
            accuracy = f"{(user_scores/max_scores):.1%}"
            company_results_user_avg.append(
                ScoreCompany(
                    user_email=user_email,
                    quiz_name=quiz_name,
                    accuracy=accuracy,
    ...
    async def get_last_employees_passage(self, company_id: int, current_user: int):
        ...
        for res in results:
            if len(last_employees_passage) == 0 or res.user_id not in [
                item.user_id for item in last_employees_passage
            ]:
                user = await self.session.execute(
                    select(User).filter(User.id == res.user_id)
                )
                user_email = user.scalars().first().email
                quiz = await self.session.execute(
                    select(Quiz).filter(Quiz.id == res.quiz_id)
                )
                quiz_name = quiz.scalars().first().name
                last_employees_passage.append(
                    LastEmployeesPassage(
                        user_id=res.user_id,
                        user_email=user_email,
                        quiz_name=quiz_name,
